...
```
  ...
  <div style={{ background: "linear-gradient(135deg, #1a0533 0%, #0f1f4d 50%, #1a0533 100%)", borderBottom: "2px solid #e3350d", padding: "20px 24px 16px", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 12, position: "relative" }}>
      ...
      <div>
        <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.5px", color: "#fff" }}>PokéStock Tracker</div>
    ...
    {hotToday.length > 0 && (
      <div style={{ marginTop: 14, background: "rgba(227,53,13,0.12)", border: "1px solid rgba(227,53,13,0.3)", borderRadius: 10, padding: "8px 14px", fontSize: 12, color: "#fca5a5", position: "relative" }}>
        🔥 <strong>Hot today:</strong> {hotToday.map(l => l.name).join(" · ")}
      </div>
    )}
  </div>
  ...
  <div style={{ padding: "20px 24px", maxWidth: 640, margin: "0 auto" }}>
      ...
      <>
        ...
        {showAdd && (
          <div style={{ background: "#151525", border: "1px solid #1e293b", borderRadius: 14, padding: 20, marginBottom: 20 }}>
            ...
            <input placeholder="Location name (e.g. Walmart - Oak Ave)" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} style={inputStyle} />
            <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} style={inputStyle}>
              {LOCATION_TYPES.map(t => <option key={t}>{t}</option>)}
            </select>
            <input placeholder="Best time to visit (e.g. 8:00 AM)" value={form.bestTime} onChange={e => setForm(f => ({ ...f, bestTime: e.target.value }))} style={inputStyle} />
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 12, color: "#64748b", marginBottom: 8 }}>Best days to visit:</div>
            ...
            <textarea placeholder="Notes (staff names, location details, tips...)" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} style={{ ...inputStyle, resize: "vertical" }} />
            <button onClick={addLocation} style={{ background: "#e3350d", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", width: "100%", marginTop: 4 }}>
              Save Location
            </button>
          </div>
        )}
        ...
        {locations.length === 0 && (
          <div style={{ textAlign: "center", color: "#475569", padding: "40px 0", fontSize: 14 }}>No locations yet. Add your first store or vending machine!</div>
        )}
          ...
          <div key={loc.id} style={{ background: "#151525", border: "1px solid #1e293b", borderRadius: 14, padding: 18, marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>{loc.type === "Vending Machine" ? "🎰" : "🏪"} {loc.type}</div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button onClick={() => deleteLocation(loc.id)} style={{ background: "none", border: "none", color: "#475569", cursor: "pointer", fontSize: 16 }}>✕</button>
              </div>
            </div>
            ...
            {loc.notes && (
              <div style={{ marginTop: 10, fontSize: 12, color: "#94a3b8", background: "#0f0f1a", borderRadius: 8, padding: "7px 10px", lineHeight: 1.5 }}>
                💬 {loc.notes.split("\n")[0]}
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
              ...
              <button onClick={() => setLogModal(loc.id)} style={{ background: "#1e293b", color: "#e2e8f0", border: "1px solid #334155", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                Log Visit
              </button>
            </div>
          </div>
    ...
    {activeTab === "stats" && (
      <div>
        ...
        {locations.length === 0 && (
          <div style={{ textAlign: "center", color: "#475569", padding: "40px 0", fontSize: 14 }}>Add locations and log visits to see stats here.</div>
        )}
        {locations.map(loc => (
          <div key={loc.id} style={{ background: "#151525", border: "1px solid #1e293b", borderRadius: 14, padding: 18, marginBottom: 14 }}>
            ...
            {loc.lastFound && <div style={{ marginTop: 8, fontSize: 11, color: "#64748b" }}>Last found stock: {loc.lastFound}</div>}
          </div>
        ))}
      </div>
    )}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, zIndex: 100 }} onClick={() => setLogModal(null)}>
      <div style={{ background: "#151525", border: "1px solid #1e293b", borderRadius: 16, padding: 24, width: "100%", maxWidth: 360 }} onClick={e => e.stopPropagation()}>
        ...
        {logForm.status === "In Stock" && (
          <input placeholder="What did you find? (e.g. Chaos Rising ETB)" value={logForm.found} onChange={e => setLogForm(f => ({ ...f, found: e.target.value }))} style={{ ...inputStyle, marginBottom: 16 }} />
        )}
        <button onClick={logVisit} style={{ background: "#e3350d", color: "#fff", border: "none", borderRadius: 10, padding: "11px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", width: "100%" }}>
          Save Log
        </button>
      </div>
    </div>
